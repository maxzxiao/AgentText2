...
export class IMessageDatabase {
    ...
    async getMessages(filter: MessageFilter = {}): Promise<MessageQueryResult> {
        ...
        if (chatId) {
            // Normalize chatId to match database format
            // For group chats: database stores GUID in chat.guid (e.g., "chat123...")
            // For DMs: database stores phone/email in chat.chat_identifier (e.g., "+1234567890")
            // API might receive: "iMessage;+;chat123..." or "chat123..." or "+1234567890" or "iMessage;+1234567890"
            let normalizedChatId = chatId
            let isGroupChat = false
            
            // Extract GUID from AppleScript format (e.g., "iMessage;+;chat123..." -> "chat123...")
            if (chatId.includes(';')) {
                const parts = chatId.split(';')
                // AppleScript group format: iMessage;+;chat...
                if (parts.length >= 3 && parts[0] === 'iMessage' && parts[1] === '+' && parts[2]?.startsWith('chat')) {
                    normalizedChatId = parts.slice(2).join(';')
                    isGroupChat = true
                } else if (parts.length >= 2 && parts[0] === 'iMessage') {
                    // Service-prefixed DM: iMessage;+1234567890 -> +1234567890
                    normalizedChatId = parts.slice(1).join(';')
                }
            } else if (chatId.startsWith('chat') && chatId.length > 10) {
                // Raw group chat GUID format
                isGroupChat = true
            }
            
            // For group chats, match against chat.guid
            // For DMs, match against chat.chat_identifier
            // Use OR to handle both cases since we don't know which format the database has
            if (isGroupChat) {
