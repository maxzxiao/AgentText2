...
- **REST API** - Complete Express server wrapper for calling methods from external scripts
...
Want to call iMessage methods from Python, cURL, or any external script? Use the built-in REST API server:
...
The server runs on `http://localhost:3000` and provides complete access to all SDK features via HTTP endpoints.
...
curl -X POST http://localhost:3000/send \
  ...
  -d '{"to":"+1234567890","content":"Hello from API!"}'
...
# Get unread messages
unread = requests.get(f"{BASE_URL}/messages/unread").json()
print(f"{unread['total']} unread messages")
```

### Available Endpoints

- **Messages**: `GET /messages`, `GET /messages/unread`
- **Send**: `POST /send`, `POST /send/batch`, `POST /send/file`, `POST /send/files`
- **Chats**: `GET /chats`
- **Watcher**: `POST /watcher/start`, `POST /watcher/stop`, `GET /watcher/stream`

**Full API Documentation**: See [API-README.md](./API-README.md) for complete endpoint documentation and examples.

**Example Scripts**:
- [TypeScript Client](./examples/api-client-example.ts) - `npm run api:example`
- [Python Client](./examples/api-client-example.py) - `python examples/api-client-example.py`
- [SSE Streaming](./examples/api-sse-example.ts) - `npm run api:sse`
- [Auto-reply Bot](./examples/api-webhook-example.ts) - `npm run api:bot`
